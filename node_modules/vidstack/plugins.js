...
const unplugin = createUnplugin((options = {}) => {
  ...
  function processModule(id, code) {
    ...
    if (elements.size) {
      if (elements.has("media-audio-layout"))
        defaultAudioLayout = true;
      if (elements.has("media-video-layout"))
        defaultVideoLayout = true;
      if (elements.has("media-plyr-layout"))
        plyrLayout = true;
  ...
  function invalidateAll() {
    ...
    for (const id of chunkToFile.keys())
      invalidateModule(id);
  }
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    switch (mode) {
      ...
      case PARSE_MODE_ELEMENT:
        if (char === " " || char === ">" || char === "\n" || char === "\r") {
          if (buffer.startsWith(mediaElementPrefix))
            elements.add(buffer);
          buffer = "";
